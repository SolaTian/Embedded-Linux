- [深入了解嵌入式应用程序](#深入了解嵌入式应用程序)
  - [嵌入式应用](#嵌入式应用)
    - [前端后端分离](#前端后端分离)
    - [前后端交互流程举例](#前后端交互流程举例)
      - [手机查询天气应用](#手机查询天气应用)
      - [工业领域的嵌入式应用](#工业领域的嵌入式应用)
  - [嵌入式应用前端实现方式之一——嵌入式 Web 服务器](#嵌入式应用前端实现方式之一嵌入式-web-服务器)
    - [嵌入式 Web 服务器、浏览器、后端 三者的关系](#嵌入式-web-服务器浏览器后端-三者的关系)
    - [常见的嵌入式 Web 服务器](#常见的嵌入式-web-服务器)
    - [以 AppWeb 为例的交互](#以-appweb-为例的交互)
      - [页面切换](#页面切换)
      - [页面保存](#页面保存)
      - [F12 浏览器开发者工具](#f12-浏览器开发者工具)
    - [Web 资源组与 AppWeb](#web-资源组与-appweb)
      - [Web 资源组和应用组典型合作流程](#web-资源组和应用组典型合作流程)
      - [页面新增显示或者逻辑配置项](#页面新增显示或者逻辑配置项)
    - [顺带插一嘴](#顺带插一嘴)
  - [什么是插件](#什么是插件)
    - [插件与应用的关系](#插件与应用的关系)
    - [插件与 Web 的关系](#插件与-web-的关系)
    - [具体交互示例](#具体交互示例)
      - [实时的视频预览](#实时的视频预览)
      - [数据/日志导出](#数据日志导出)
      - [固件升级（文件导入）](#固件升级文件导入)
      - [换一种思路——绕过插件](#换一种思路绕过插件)
...
3. AppWeb 直接返回 `page2.html` 文件内容给浏览器，不涉及后端业务逻辑。
...
实际在开发的时候，应用后端有时可能不需要增加新的路由，只要复用之前已经存在的路由，增加一个字段即可。不过要注意这个字段 Web 资源组也需要同步增加。


### 顺带插一嘴

这里顺带插一嘴，为啥可以同时登录多个 Web 页面进行操作呢？

其实不光针对于嵌入式的 Web 应用，还有一些其他的 Web 应用，主要在于以下几点原因：
- HTTP协议本身无状态：Web 应用基于 HTTP 协议，每次请求独立处理，服务器默认不会保存用户状态，会话状态通常通过 Cookies、Session ID 或 Token 等在客户端维护，服务器不会强制绑定单一连接。
- Web 会话管理：用户登录后，服务器生成 Session ID 或 Token 并存储在客户端（如浏览器）后续请求携带此ID进行验证。多设备登录时，每个会话独立，服务器不检查会话冲突。


就比如我在两个浏览器上登录两个一样的 GitHub 是可以的，同时登录两个 嵌入式的 Web 也是可以的。
...
> 插件：是一种可动态加载的软件模块，用于扩展主程序的功能，而无需修改主程序本身的代码。

插件的核心特点：
- 模块化：按需加载，灵活禁用或启用
- 独立性：与主程序解耦，通常通过标准接口通信
- 专用性：专注于特定功能    

### 插件与应用的关系

插件就是为了扩展主程序的功能，一般都是由主程序进行调用的。一般都是应用是“调用者”，插件是“执行者”。

应用会定义插件的接口规范，比如（命令号和数据结构），并通过接口与插件进行通信。

插件是功能的具体执行者，专注于单一的任务，不直接操作全局资源，而是通过应用提供的接口获取数据

### 插件与 Web 的关系

插件用于实现 Web 无法直接完成的功能，例如：
- 视频监控：
  - Web 页面：显示视频控件和云台控制按钮
  - 需要插件通过 RTSP/ONVIF 协议从设备拉流，解码后渲染到浏览器
- 固件升级：
  - Web 页面：提供升级文件上传界面
  - 插件：通过 USB/UART 将固件写入设备 Flash。

插件主要用于弥补浏览器在硬件访问、私有协议支持等方面的不足。

### 具体交互示例

#### 实时的视频预览

在一些实时监控的场景，往往会有在线预览的场景

1. 当用户在页面上点击“预览”按钮时，Web 页面会触发 `JavaScript`代码检查是否有插件，有则唤醒插件，没有则有可能报出安装插件的提示
2. 插件直接与设备通信（如通过 RTSP/RTP 协议拉取视频流）
3. 若视频流没有解码，则需要插件进行解码
4. 解码后将视频渲染到页面 `<video>` 标签


#### 数据/日志导出

在一些嵌入式设备上，往往有导出数据和日志的功能，这些功能具体的实现流程是怎样的呢？

1. 当用户在页面上点击“导出”按钮时，Web 页面会触发 `JavaScript`代码检查是否有插件，有则唤醒插件，没有则有可能报出安装插件的提示
2. 插件此时作为 TCP 客户端连接到嵌入式应用中已经指定好的端口
3. 插件发送与应用预先约定好的命令号，如 NET_EXPORT
4. 应用根据接收到的命令号执行响应的逻辑函数
5. 应用将数据或者日志从硬盘文件系统或者数据库中读取出来
6. 应用按照与插件预先约定好的协议发送数据包
7. 插件接收到数据之后调用其接口如`fwrite()`等接口将接收到的数据写入指定路径或者直接写文件（这一过程会弹出对话框让用户选择保存位置）
8. 插件的命名方式通过时间戳或者应用蹄冻的信息生成文件名


由于是 TCP 通信，所以有时可能需要抓包，插件运行在电脑上，因此使用的电脑的 IP 地址，如果向抓设备和插件之间交互的包，那么就可以
- 在设备底层使用 tcpdump 抓对应网卡和目标主机为电脑的包
  
        tcpdump -i any host 电脑IP -w zhuabao.pcap -v
- 在电脑上使用 wireshark 抓对应网卡的目标主机为设备的包
  

#### 固件升级（文件导入）<a id="固件升级（文件导入）"></a>

嵌入式设备经常会出现 Web 升级更新版本的情况。具体是如下的流程

1. 客户端浏览器操作 Web 页面，点击“浏览”按钮。Web 页面会触发 `JavaScript`代码检查是否有插件，有则唤醒插件，没有则有可能报出安装插件的提示
2. 插件跳出窗口选择本地电脑上的文件。Web 与插件交互，调用插件方法传递固件数据
3. 插件通过 TCP/IP 协议发送固件到设备的升级接口
4. 设备会校验固件的完整性，语言，加密型号等，如果满足校验。擦除 Flash 并写入新的固件
6. 通过 `JavaScript`回调或者`WebSocket`通知进度
7. Web 经过渲染，显示升级进度
8. 升级完成 100% 之后设备内部下发重启命令
9. 通过 `JavaScript`回调或者`WebSocket`通知升级完成
10. Web 经过渲染，显示设备重启


#### 换一种思路——绕过插件

上面的两个场景都涉及到了插件，那么请问可不可以直接跳过插件，仅用前端和应用之间的交互呢？

当然是可以的，一个个来分析，实时预览，客户端浏览器直接通过 `JavaScript`发起播放请求到 AppWeb , 新增对应的路由和逻辑处理函数（或者与 Web 预定好新增字段），从设备中拉取视频流（如通过 RTSP），转码成浏览器兼容模式（如 HLS/MP4 ），最后在通过 AppWeb 使用 HTTP/WebSocket 推送到浏览器，浏览器使用`<video>` 标签播放转码后的流。


再来看导出这个，换一种思路，客户端不点击“导出“，直接点一下数据查询按钮或者其他的客户端与 AppWeb 交互的按钮，直接在对应的路由处理函数里面将需要导出的数据预处理好（比如搜索，读取，写文件）之后将处理好的数据，用表单的形式将要导出的数据或者日志通过 AppWeb 返回给客户端，浏览器将文件进行下载。这种方式就是绕过了插件，但是这种方式 Web 资源组和应用组都需要在原先支持的路由函数里面新增对应的处理逻辑。