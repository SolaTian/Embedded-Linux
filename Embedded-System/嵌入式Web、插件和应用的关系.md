...
用户点击预览按钮 → 前端发起请求 → 后端启动视频流处理 → 插件负责视频编码和传输 → 前端接收并播放视频流。

与之前不涉及插件的交互类似

- 客户端浏览器点击预览之后，Web 发起请求
- AppWeb 在接收到对应的请求后调用后端对应的逻辑处理函数，如预览相关的函数
- 后端与插件建立起 socket 连接，将设备中经过解码后的视频流发送给插件
- 插件接收到视频流之后，可能会对流进行编码或者传输（注意这时就不像普通的请求通过 AppWeb 进行返回了，即不是通过普通的 HTTP 请求响应模型了，而是通过持续连接，比如 WebSocket 或者流协议 RTMP 进行返回 ）
- 插件将视频流传输到 Web ，Web 比如通过 `JS/HTML`等将视频进行播放。

#### 数据导出


用户点击导出数据按钮 -> 前端发起请求 -> 后端开始搜索数据库 -> 将数据发送给插件

用户点击导出数据按 → 钮应用收到前端操作 → 解析为命令号 → 查询数据库 → 将数据传递给插件 → 插件执行逻辑 → 返回结果给应用。